...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_vitals'])) {
    ...
    // Security check: Ensure patient is active (Admitted or in Queue today)
    $is_active = db_select_one("
        SELECT p.id 
        FROM patients p
        ...
        WHERE p.id = $1 AND (a.id IS NOT NULL OR app.id IS NOT NULL)
    ", [$v_patient_id]);

    if (!$is_active) {
        $_SESSION['error_msg'] = "Cannot record vitals: Patient is not currently admitted or in today's queue.";
