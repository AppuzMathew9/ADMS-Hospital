...
$doctors = db_select("SELECT id, first_name, last_name, specialization, department_id FROM staff WHERE role = 'doctor' ORDER BY first_name");
                    ...
                    <form method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                ...
                                <select name="doctor_id" id="doctor_select" class="form-control" onchange="filterDepartments()">
                                    <option value="" data-dept-id="">-- None --</option>
                                    <?php foreach ($doctors as $d): ?>
                                        <option value="<?php echo $d['id']; ?>" data-dept-id="<?php echo $d['department_id']; ?>" <?php echo ($edit_data['doctor_id'] == $d['id']) ? 'selected' : ''; ?>>
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                ...
                                <select name="department_id" id="dept_select" class="form-control">
...
function filterDepartments() {
    ...
    // If a doctor is selected and has a department
    if (deptId && deptId !== "") {
        // Loop through departments and show only matching one
        let found = false;
        for (let i = 0; i < deptSelect.options.length; i++) {
            const opt = deptSelect.options[i];
            if (opt.value === "" || opt.value === deptId) {
                opt.style.display = "";
                if (opt.value === deptId) {
        ...
        // Show all if no doctor selected
        for (let i = 0; i < deptSelect.options.length; i++) {
...
document.addEventListener('DOMContentLoaded', function() {
    // We don't want to auto-select if we are in edit mode and a different dept was saved
    // but the filter should still apply
    const doctorSelect = document.getElementById('doctor_select');
    if (doctorSelect.value !== "") {
        // Apply filter but don't force change selection if already set
        const deptId = doctorSelect.options[doctorSelect.selectedIndex].getAttribute('data-dept-id');
        const deptSelect = document.getElementById('dept_select');
        if (deptId) {
            for (let i = 0; i < deptSelect.options.length; i++) {
                const opt = deptSelect.options[i];
